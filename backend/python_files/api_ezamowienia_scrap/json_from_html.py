...
def process_all_notices(
    input_file: str = "html_bodies.json",
    output_file: str = "parsed_notices.json",
    text_output_file: str = "parsed_notices.txt",
) -> list[dict]:
    """Przetwarza wszystkie ogłoszenia z pliku JSON."""
    logger.info("Wczytywanie pliku %s...", input_file)
    ...
    for idx, item in enumerate(items):
        ...
        if not html_content:
            logger.warning("Pominięto %s - brak treści HTML", notice_id)
            continue
        
        logger.info("Parsowanie ogłoszenia %s...", notice_id)
    ...
    logger.info("Zapisywanie %s...", output_file)
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(all_parsed, f, ensure_ascii=False, indent=2)
    
    logger.info("Zapisywanie %s...", text_output_file)
    with open(text_output_file, "w", encoding="utf-8") as f:
        f.write('\n'.join(all_text_outputs))
    
    logger.info("Przetworzono %d ogłoszeń. JSON: %s, Tekst: %s",
                len(all_parsed), output_file, text_output_file)
    
    return all_parsed


def parse_single_html(html_content: str) -> dict:
...
def get_all_field_numbers(parsed_notices: list[dict]) -> list[str]:
    ...
    # Sortuj naturalnie
    def _sort_key(field_num: str) -> tuple[int, ...]:
        parts = field_num.split('.')
        return tuple(int(p) for p in parts)
    
    return sorted(all_fields, key=_sort_key)


def generate_field_coverage_report(parsed_notices: list[dict]) -> None:
    ...
    logger.info("RAPORT POKRYCIA PÓL (%d ogłoszeń)", total_notices)
    
    def _sort_key(item: tuple[str, int]) -> tuple[int, ...]:
        field_num = item[0].split(' - ')[0]
        parts = field_num.split('.')
        return tuple(int(p) for p in parts)
    
    for key, count in sorted(field_counts.items(), key=_sort_key):
        pct = count / total_notices * 100
        bar = '█' * int(pct / 5) + '░' * (20 - int(pct / 5))
        logger.info("  %60s %4d/%d (%5.1f%%) %s", key, count, total_notices, pct, bar)
...
if __name__ == "__main__":
    ...
    if parsed:
        ...
        logger.info("Wszystkie znalezione numery pól (%d):", len(all_fields))
        for f in all_fields:
            logger.info("  %s", f)